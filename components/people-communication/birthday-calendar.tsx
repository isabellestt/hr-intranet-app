...
import { BirthdayEvents, type BirthdayEvent, type EventCategory } from '@/lib/mock-data'
...
function groupByDate(events: BirthdayEvent[]): Record<string, BirthdayEvent[]> {
  return events.reduce<Record<string, BirthdayEvent[]>>((acc, e) => {
...
function EventBadge() {
  return (
    <span
      ...
      style={{ backgroundColor: BIRTHDAY_STYLE.bg, color: BIRTHDAY_STYLE.text }}
    >
      Birthday
    </span>
  )
...
export default function CompanyCalendar() {
  ...
  const eventMap    = useMemo(() => groupByDate(BirthdayEvents), [])
  ...
  }, [year, month, daysInMonth, eventMap])
  ...
  return (
    <div className="p-6 min-h-full bg-[#F5F4F0]">
      ...
      <div className="flex items-end justify-between mb-6 pb-5 border-b border-[#E5E2DC]">
        <div>
          <h2 className="font-serif text-2xl font-semibold text-[#1B2A4A]">Birthday Calendar</h2>
        ...
        <div className="bg-white border border-[#E5E2DC] p-6">
          ...
          <div className="grid grid-cols-7 gap-0.5">
            {cells.map((day, i) => {
              ...
              const hasBirthday = evts.length > 0
              // const hasHoliday = filtered.some(e => e.category === 'Holiday')

              // Up to 3 dot indicators
              // const dots = filtered.slice(0, 3)
              const showDot = evts.length > 0
              ...
              return (
                <button
                  ...
                  `}
                  style={
                    !isSelected && hasBirthday
                      ? {
                          backgroundColor: BIRTHDAY_STYLE.bg,
                          color: BIRTHDAY_STYLE.text,
                        }
                      : undefined
                  }
                >
                    ...
                    <div className="flex gap-0.5 flex-wrap justify-center">
                      {dots.map(e => (
                        <span
                          key={e.id}
                          className="w-1.5 h-1.5 rounded-full"
                          style={{ backgroundColor: isSelected ? 'rgba(255,255,255,0.7)' : CATEGORY_STYLES["Bir"].dot }}
                        />
                      ))}
                    </div>
                  )} */}
          ...
          <div className="flex flex-wrap gap-4 mt-5 pt-4 border-t border-[#E5E2DC]">
            <div className="flex items-center gap-1.5 text-xs font-sans text-[#777]">
              <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: BIRTHDAY_STYLE.dot }} />
              {BIRTHDAY_STYLE.label}
            </div>
          </div>
        </div>
                ...
                {selectedEvents.map(event => {
                  return (
                    <div
                      ...
                      style={{ borderLeftColor: BIRTHDAY_STYLE.dot, backgroundColor: "#edf8f9ff"}}
                    >
                      <div className="flex items-start justify-between gap-2 mb-1">
                        <p className="font-sans text-sm font-semibold text-[#1B2A4A] leading-snug">{event.name}</p>
                        <EventBadge />
                      </div>
                      <div className="flex items-center gap-1.5 mt-1.5">
                        <span className="font-sans text-xs text-[#666]">Happy Birthday {event.name}!</span>
                      </div>
                      {/* {event.time && (
                        <div className="flex items-center gap-1.5 mt-1.5">
                      ...
                      {event.department && (
                        <p className="font-sans text-xs text-[#888] mt-2 leading-relaxed">{event.department}</p>
                      )}
                    </div>
                  )
                })}
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-sans text-xs font-semibold text-[#1B2A4A] truncate">{event.name}</p>
                        {event.department && (
                          <p className="font-sans text-xs text-[#AAA] mt-0.5">{event.department}</p>
                        )}
                      </div>
                      {/* Category dot */}
                      <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: BIRTHDAY_STYLE.dot }} />
                    </button>
                  )
                })}
              </div>
